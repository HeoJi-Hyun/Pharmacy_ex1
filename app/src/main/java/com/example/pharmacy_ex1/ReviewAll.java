...
// 어떤 값을 선택했는지 토스트를 뿌려줌
                Toast.makeText(ReviewAll.this, adapterView.getItemAtPosition(i)+ " 클릭", Toast.LENGTH_SHORT).show();

// 게시물의 번호와 userid를 가지고 DetailActivity 로 이동
                Intent intent = new Intent(ReviewAll.this, Review_Ditail.class);
...
// 버튼 컴포넌트 초기화
        ibtw = findViewById(R.id.ibtw);

// 버튼 이벤트 추가
        ibtw.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {

// userid 를 가지고 RegisterActivity 로 이동
                Intent intent = new Intent(ReviewAll.this, RePencle.class);
                intent.putExtra("userid", userid);
                startActivity(intent);
            }
        });
...
// ListView 에서 사용할 arrayAdapter를 생성하고, ListView 와 연결
                ArrayAdapter arrayAdapter = new ArrayAdapter<String>(ReviewAll.this, android.R.layout.simple_list_item_1, titleList);
                all_List.setAdapter(arrayAdapter);
