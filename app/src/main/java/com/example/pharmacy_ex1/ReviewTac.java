...
public class ReviewTac extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.reviewtac);

// LoginActivity 에서 넘긴 userid 값 받기
        userid = getIntent().getStringExtra("userid");

        // 컴포넌트 초기화
        tac_List = findViewById(R.id.tac_List);

        // listView 를 클릭했을 때 이벤트 추가
        tac_List.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {

// 어떤 값을 선택했는지 토스트를 뿌려줌
                Toast.makeText(ReviewTac.this, adapterView.getItemAtPosition(i)+ " 클릭", Toast.LENGTH_SHORT).show();

// 게시물의 번호와 userid를 가지고 DetailActivity 로 이동
                Intent intent = new Intent(ReviewTac.this, Review_Ditail.class);
...
// 버튼 컴포넌트 초기화
        ibtntac = findViewById(R.id.ibtntac);

// 버튼 이벤트 추가
        ibtntac.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {

// userid 를 가지고 RegisterActivity 로 이동
                Intent intent = new Intent(ReviewTac.this, RePencle.class);
                intent.putExtra("userid", userid);
                startActivity(intent);
            }
        });
...
// ListView 에서 사용할 arrayAdapter를 생성하고, ListView 와 연결
                ArrayAdapter arrayAdapter = new ArrayAdapter<String>(ReviewTac.this, android.R.layout.simple_list_item_1, titleList);
                tac_List.setAdapter(arrayAdapter);
