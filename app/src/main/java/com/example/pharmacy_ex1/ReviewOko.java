...
public class ReviewOko extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.reviewoko);

// LoginActivity 에서 넘긴 userid 값 받기
        userid = getIntent().getStringExtra("userid");

        // 컴포넌트 초기화
        oko_List = findViewById(R.id.oko_List);

        // listView 를 클릭했을 때 이벤트 추가
        oko_List.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {

// 어떤 값을 선택했는지 토스트를 뿌려줌
                Toast.makeText(ReviewOko.this, adapterView.getItemAtPosition(i)+ " 클릭", Toast.LENGTH_SHORT).show();

// 게시물의 번호와 userid를 가지고 DetailActivity 로 이동
                Intent intent = new Intent(ReviewOko.this, Review_Ditail.class);
...
// 버튼 컴포넌트 초기화
        ibtnoko = findViewById(R.id.ibtnoko);

// 버튼 이벤트 추가
        ibtnoko.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {


// userid 를 가지고 RegisterActivity 로 이동
                Intent intent = new Intent(ReviewOko.this, RePencle.class);
                intent.putExtra("userid", userid);
                startActivity(intent);
            }
        });
...
// ListView 에서 사용할 arrayAdapter를 생성하고, ListView 와 연결
                ArrayAdapter arrayAdapter = new ArrayAdapter<String>(ReviewOko.this, android.R.layout.simple_list_item_1, titleList);
                oko_List.setAdapter(arrayAdapter);
